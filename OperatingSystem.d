...
version(STDIO)
{
    import std.stdio;
    import std.file;
    import FileSystem : FileCompat, CWD, cwd, BLOCK, DISK, KFS, from_block, touch, exists;
...
public:
    ...
    void set_disk(string path)
    {
        m_disk_path = path;
    }

    int shell(std.stdio.File instream, bool e = false) {
        version(KaiFS) {
            stderr.writef("Opening %s\n", m_disk_path);
            DISK = std.stdio.File(m_disk_path, "r+");
        }
        stderr.writef("error1: %d\n", DISK.error);
        if(exists(std.file.getcwd()))
        {
            chdir(std.file.getcwd());
        }
        else
        {
            stderr.writef("error2: %d\n", DISK.error);
            chdir("/");
        }
        echo = e;
